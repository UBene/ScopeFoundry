...
class SweepNDBase(Measurement, ABC):
    ...
    def run(self):
        ...
        if self.should_check_actuators() and not actuators:
    ...
    def setup(self):
        ...
        s.New(
            name="scan_mode",
            dtype=str,
            initial=self.get_default_scan_mode(),
        ...
        s.New(
            name="data_set",
            dtype=str,
            initial=self.get_default_data_set_name(),
            choices=("",),
            description=self.get_data_set_description(),
        ).add_listener(self.update_display)
        s.New("average_over_repetitions", dtype=bool, initial=True).add_listener(
            self.update_display
        )
        
        # Add average_to_scan_shape for 1D only (child can override)
        if hasattr(self, 'add_average_to_scan_shape') and self.add_average_to_scan_shape:
    ...
    def get_scan_modes_description(self):
        ...
        raise NotImplementedError("Child class must implement get_scan_modes_description()")
    ...
    def get_default_scan_mode(self):
        """Return the default scan mode for this sweep type."""
        return "co-move"

    def get_default_data_set_name(self):
        """Return the default data set name for this sweep type."""
        return ""

    def get_data_set_description(self):
        """Return the data set description for this sweep type."""
        return "data set to plot"
    ...
    def should_check_actuators(self):
        """Return whether to check for empty actuators list. Override in child classes if needed."""
        return True
